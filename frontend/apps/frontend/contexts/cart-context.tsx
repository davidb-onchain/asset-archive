...
const STORAGE_OLD_KEY = "local_cart_v1"
...
export function CartProvider({ children }: { children: React.ReactNode }) {
  ...
  // Persist to storage (write to new key only) - only after hydration
  useEffect(() => {
  ...
  const addItem = useCallback(async (item: Omit<CartItem, "cartLineId" | "quantity">, quantity = 1) => {
    // Simulate loading for consistency with existing UX
    dispatch({ type: "SET_LOADING", payload: true })
    dispatch({ type: "SET_ERROR", payload: null })
    try {
      // Apply optimistic addition; quantity is respected only for new lines
      for (let i = 0; i < Math.max(1, quantity); i++) {
        dispatch({ type: "ADD_ITEM_OPTIMISTIC", payload: item })
      }
    } catch (err) {
      console.error("Failed to add item to local cart:", err)
  ...
  const updateItemQuantity = useCallback(async (cartLineId: string, quantity: number) => {
    ...
    try {
      ...
      console.error("Failed to update quantity in local cart:", err)
