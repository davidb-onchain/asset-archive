...
import { Search, Star, User, Menu, Settings } from "lucide-react"
...
export function Navbar() {
  ...
  const [showCmsIcon, setShowCmsIcon] = useState(false)
  const { state, dispatch } = useCart()

  // Get dynamic store name
  const storeName = getStoreName()

  // Check if CMS is configured (after hydration to avoid SSR mismatch)
  useEffect(() => {
    setShowCmsIcon(isStrapiConfigured())
  }, [])
      ...
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center space-x-4">
            {/* CMS Setup Icon - hidden in demo mode */}
            {process.env.NEXT_PUBLIC_DEMO_MODE !== "true" && (
            <a href="/setup" className="p-2 hover:bg-gray-100 rounded-full transition-colors" title="Setup CMS">
          ...
          <div className="md:hidden border-t border-gray-200 py-4">
            <div className="flex flex-col space-y-4">
              <div className="relative">
                <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" />
                <Input type="search" placeholder="Search assets..." className="pl-10 pr-4 py-2 w-full" />
              </div>
              <a
                href="/"
